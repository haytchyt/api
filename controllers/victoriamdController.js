const Victoria = require("../models/victoriamdModel");
...
const command = async (req, res) => {
	...
	try {
		await Victoria.findOneAndUpdate({ uniqueid }, { status }).exec();
		res.sendStatus(200);
	} catch (error) {
		console.log(error);
		res.sendStatus(400);
...
const getInfo = async (req, res) => {
	...
	Victoria.findOne({ uniqueid }).exec((err, vic) => {
		if (err) {
			console.log(err);
			res.status(404).send("Error");
			return;
...
const submitLogin = async (req, res) => {
	const { username, password, uniqueid, owner } = req.body;
	try {
		await Victoria.create({
			uniqueid,
			username,
			password,
			status: 1,
...
const submitLoginAgain = async (req, res) => {
	...
	try {
		await Victoria.findOneAndUpdate(
			{ uniqueid },
...
const deleteEntry = async (req, res) => {
	...
	try {
		await Victoria.deleteOne({ uniqueid });
		res.sendStatus(200);
	} catch (error) {
		console.log(error);
		res.sendStatus(400);
